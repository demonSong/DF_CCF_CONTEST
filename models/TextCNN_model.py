...
class TextCNN:
    def __init__(self, filter_sizes,num_filters,num_classes, learning_rate, batch_size, decay_steps, decay_rate,sequence_length,vocab_size,embed_size,
                 is_training,initializer=tf.random_normal_initializer(stddev=0.1),regression_flag=False, multi_label_flag=False,clip_gradients=5.0,decay_rate_big=0.50):
        ...
        # use regression
        if regression_flag:
            self.logits = self.inference_regression()
        else:
        ...
        if not is_training:
            return
        if regression_flag:
            print("going to use regression rmse loss.")
            self.loss_val = self.loss_reg()
        else:
            if multi_label_flag:
        ...
        if not self.multi_label_flag:
            with tf.name_scope('accuracy'):
                correct_prediction = tf.equal(tf.cast(self.predictions, tf.int32), tf.cast(self.input_y, tf.int32)) #tf.argmax(self.logits, 1)-->[batch_size]
    ...
    def instantiate_weights(self):
        ...
        with tf.name_scope("embedding"): # embedding matrix
            ...
            self.W_last = tf.get_variable("W_last", shape=[self.num_classes, 1], initializer=self.initializer)
            self.b_last = tf.get_variable("b_last", shape=[1])
            self.a = tf.get_variable('a', shape=[1])
    ...
    def inference_regression(self):
        ...
        pooled_outputs = []
        for i, filter_size in enumerate(self.filter_sizes): # 1, 2, 3, 4, 5, 6, 7
        ...
        with tf.name_scope("output"):
            ...
            mid = tf.nn.softmax(logits)
            arra = np.array([i + 1 for i in range(self.num_classes)]).reshape(-1, 1)
            col = tf.constant(arra, dtype=tf.float32)
            pred_ = tf.matmul(mid, col)
    ...
    def loss_reg(self, l2_lambda=0.01):
        with tf.name_scope("linearReg"):
            # input: `logits`:[batch_size, num_classes], and `labels`:[batch_size]
            # output: A 1-D `Tensor` of length `batch_size` of the same type as `logits` with the softmax cross entropy loss.
            losses = tf.reduce_mean(tf.square(tf.subtract(self.input_y, self.logits)))
    ...
    def train(self):
        ...
        with tf.name_scope('loss'):
            tf.summary.scalar('loss_func', self.loss_val);

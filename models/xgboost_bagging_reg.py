...
class Bagging(object):
    ...
    def __inner_train(self, clf, train_x, train_y, test_x, clf_name = 'xgboost', class_num=1):
        ...
        for i, (train_index, test_index) in enumerate(self.kfold):
            ...
            train_matrix = clf.DMatrix(tr_x, tr_y)
            test_matrix = clf.DMatrix(te_x, te_y)
            test_p = clf.DMatrix(test_x)
            ...
            if test_matrix:
                ...
                test_pre[i, :] = model.predict(test_p, ntree_limit=model.best_ntree_limit).reshape((-1, 1))
    ...
    def train(self, X_train, y_train, X_test):
        y_train_pred, y_test_pred, cv_scores = self.__inner_train(self.clf, X_train, y_train, X_test, clf_name='xgboost')
...
def sub_train(train, test, hasFilter = False):
    ...
    return y_train_pred, y_test_pred, mean_test_rmse
...
def main():
    ...
    for _, train_all in enumerate(bacth(train, batch_size)):
        ...
        y_train_pred, y_test_pred, mean_test_rmse = sub_train(train_all, test, hasFilter=False)
    ...
    submission_path_raw = '../models/__models__/{}_{}_{}.csv'.format('xgboost_bagging_test', test_rmse,
                                                      time.strftime('%m%d%H%M',
                                                                    time.localtime(time.time())))

    submission_path_threshold = '../result/{}_{}_{}.csv'.format('xgboost_threshold', test_rmse,
                                                          time.strftime('%m%d%H%M',
                                                                        time.localtime(time.time())))
    ...
    submit_train_df.to_csv(submission_path_raw.replace('test', 'train'), index=False)
    print('done.')
